...
def write_jsonl(annotations, path):
...
class Captioner:
	def __init__(self, args):		
		self.args = args
		self.annotation_db = load_jsonl(args.input_file)
		self.load_model()

	def load_model(self):
		# Load model
		checkpoint = torch.load(self.args.model) # , map_location=str(device))
	...
	def caption_image_beam_search(self, image_path):
		"""
	...
	def process_output(self, seq):
		words = [self.rev_word_map[k] for k in seq]
		# Take at the most 50 words
		words = words[:50]
		if words[0] == '<start>':
			words = words[1:]
		if words[-1] == '<end>':
	...
	def run(self):
		...
		for ann in tqdm(self.annotation_db):
			...
			image_path = os.path.join(self.args.image_dir, ann['img'])
			seq, _ = self.caption_image_beam_search(image_path)
			# alphas = torch.FloatTensor(alphas)
			caption_text = self.process_output(seq)
			# print(caption_text)
			# print("{:.2f} seconds".format(time.time() - start_time))
			
			# Visualize caption and attention of best sequence
			# visualize_att(args.img, seq, alphas, rev_word_map, args.smooth)
			ann['caption_text'] = caption_text
			output_annotations.append(ann)
		# write to another json_l
		write_jsonl(output_annotations, self.args.output_file)

if __name__ == '__main__':
	parser = argparse.ArgumentParser(description='Generation Captions from Images')

	parser.add_argument('--input_file', required=True, help='path to input annotations file')
	parser.add_argument('--output_file', required=True, help='path to file to write annotations')
